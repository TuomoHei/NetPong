...
public class Server : MonoBehaviour, INetEventListener
{
    ...
    public void OnPeerConnected(NetPeer peer)
    {
        ...
        // if connections == 2 send packet "start game"
        if(playersDictionary.Count == 2)
        {
            netDataWriter.Reset();
            netDataWriter.Put((int)NetworkTags.StartGame);
            foreach (var client in playersDictionary)
            {
                client.Value.NetPeer.Send(netDataWriter, DeliveryMethod.ReliableUnordered);
            }
            netDataWriter.Reset();
        }
